    ...
    <mixed-citation>
      ...
      <xsl:choose>
        ...
        <xsl:when test="@publication-type='thesis'
          ...
          <xsl:call-template name="thesis-citation-content"/>
        </xsl:when>
...
<!-- ============================================================= -->
  ...
  <xsl:template name="fallback-citation-content">
    ...
    <xsl:call-template name="format-names">
      <xsl:with-param name="names" select="$placed-names except comment"/>
    ...
    <xsl:call-template name="comma-sequence-sentence">
      <xsl:with-param name="phrases" as="node()*">
        <xsl:apply-templates select="* except person-group" mode="format"/>
      </xsl:with-param>
    </xsl:call-template>
  </xsl:template>
...
<!-- ============================================================= -->
<!-- For publication types 'thesis' and 'confproc'               -->
<!-- ============================================================= -->

  <xsl:template name="thesis-citation-content">
    <xsl:variable name="lang" select="@xml:lang" />
      ...
      <xsl:call-template name="comma-sequence-sentence">
        <xsl:with-param name="phrases" as="node()*">
          <xsl:apply-templates mode="article-item"
            select="* except ($placed-names | $date | $titles | $conference)"/>
        </xsl:with-param>
      </xsl:call-template>
  </xsl:template>
...
<!-- 'author' mode formats structured name elements for the 
  ...
  <xsl:template match="name" mode="author">
    <xsl:param name="lang"></xsl:param>
    <named-content content-type="author-name">
    <xsl:choose>
      <xsl:when test="$lang='ko'">
          <xsl:apply-templates select="surname" mode="format"/><xsl:apply-templates select="given-names" mode="format"/>
...
<!-- "format" mode: maps arbitrary citation element contents into
  ...
  <xsl:template mode="format" match="source">
    <named-content content-type="source">
      <xsl:apply-templates mode="format"/>
    </named-content>
  </xsl:template>
