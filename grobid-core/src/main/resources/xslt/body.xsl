...
<xsl:stylesheet version="1.0"
  ...
  <xsl:output method="html" encoding="UTF-8" use-character-maps="html-illegal-chars" />
  ...
  <xsl:param name="render-mode" select="'pubreader'"/>
...
<!--         <xsl:apply-templates mode="metadata" select="author-notes"/> -->
<!--       </xsl:if> -->
    </xsl:for-each>
...
<!--     		<li> -->
				<!-- change context to front/article-meta (again) -->
				<xsl:if test="article-meta/author-notes">
					<xsl:for-each select="article-meta | self::front-stub">
						<!-- back in article-meta or front-stub context	-->
						<xsl:if test="author-notes">
							<xsl:apply-templates mode="metadata" select="author-notes"/>
...
<!--     		<li> -->
		        ...
		        <xsl:if test="article-meta/volume">
		          <xsl:text>, </xsl:text>
		  	      <span class="volume">
		  	        <span class="article-volume">Vol.</span><xsl:value-of select="article-meta/volume" />
		  	      </span>
		        </xsl:if>
		        <xsl:if test="article-meta/issue">
		          <xsl:text>, </xsl:text>
		          <span class="issue">
		            <span class="article-issue">No.</span><xsl:value-of select="article-meta/issue" />
		          </span>
		        </xsl:if>
		
		        <xsl:if test="article-meta/fpage | article-meta/lpage | article-meta/page-range">
		          <xsl:text>, </xsl:text>
		          <span class="page">
		            <span class="article-page">pp.</span>
		            <xsl:value-of select="article-meta/fpage"/>
		            <xsl:if test="normalize-space(string(article-meta/lpage[not(.=../fpage)]))">
		              <xsl:text>-</xsl:text>
		              <xsl:value-of select="article-meta/lpage"/>
		            </xsl:if>
		            <xsl:for-each select="article-meta/page-range">
		              <xsl:value-of select="."/>
		            </xsl:for-each>
		          </span>
		        </xsl:if>
...
<!--     		<li> -->
    		...
    		<xsl:if test="article-meta/article-id[@pub-id-type='doi']">
		      <xsl:variable name="doc-doi" select="article-meta/article-id[@pub-id-type='doi']/text()" />
		      <a href="https://doi.org/{$doc-doi}" class="doi_link" target="_blank">https://doi.org/<xsl:value-of select="$doc-doi" /></a>
	        </xsl:if>
		    <xsl:if test="not(article-meta/article-id[@pub-id-type='doi']) and $doi">
		      <p class="pub-id-doi"><a href="https://doi.org/{$doi}" class="doi" target="_blank">https://doi.org/<xsl:value-of select="$doi" /></a></p>
		    </xsl:if>
<!--     		</li> -->
<!--     	</ul> -->
    ...
    <xsl:if test="/article/@xml:lang='ko'">
	    <xsl:for-each select="article-meta | self::front-stub">
	      <xsl:if test="abstract | trans-abstract">
	        <section class="abstract en">
	        <xsl:choose>
	          <xsl:when test="trans-abstract[@xml:lang='en']">
		          <xsl:for-each select="trans-abstract[@xml:lang='en']">
		            <xsl:call-template name="named-anchor"/>
		              <h2 class="article_title">
			            <span><xsl:text>Abstract</xsl:text></span>
			          </h2>
		            <xsl:apply-templates select="*[not(self::title)]"/>
		          </xsl:for-each>
	          </xsl:when>
	          <xsl:otherwise>
		          <xsl:for-each select="abstrat">
		            <xsl:call-template name="named-anchor"/>
		            <h2 class="article_title">
	                <span><xsl:text>Abstract</xsl:text></span>
	                </h2>
			...
			<div class="keyword_box">
				<dl>
					<xsl:if test="kwd-group[@xml:lang='en']">
						<dt>keyword</dt>
						<dd>
							<xsl:call-template name="named-anchor"/>
							<xsl:for-each select="kwd-group[@xml:lang='en']/kwd">
								<span><span class="kwd"><xsl:apply-templates /></span></span>
							</xsl:for-each>
						</dd>
					</xsl:if>
				</dl>
	      ...
	      <xsl:if test="abstract">
	        <section class="abstract ko">
	          <h2 class="article_title">
		  	    <span><xsl:text>초록</xsl:text></span>
	          </h2>
	        ...
	        <div class="keyword_box">
				<dl>
					<xsl:if test="kwd-group[not(@xml:lang='en')]">
						<dt>주요 용어</dt>
						<dd>
							<xsl:call-template name="named-anchor"/>
							<xsl:for-each select="kwd-group[not(@xml:lang='en')]/kwd">
								<span><span class="kwd"><xsl:apply-templates /></span></span>
							</xsl:for-each>
						</dd>
					</xsl:if>
    ...
    <xsl:if test="/article/@xml:lang='en'">
	    <xsl:for-each select="article-meta | self::front-stub">
	      <xsl:if test="abstract">
	        ...
	        <div class="keyword_box">
				<dl>
					<xsl:if test="kwd-group[not(@xml:lang='ko')]">
						<dt>keyword</dt>
						<dd>
							<xsl:call-template name="named-anchor"/>
							<xsl:for-each select="kwd-group[not(@xml:lang='ko')]/kwd">
								<span><span class="kwd"><xsl:apply-templates /></span></span>
							</xsl:for-each>
						</dd>
					</xsl:if>
	      ...
	      <xsl:if test="abstract | trans-abstract">
	        <section class="abstract ko">
	        <xsl:choose>
	          <xsl:when test="trans-abstract[@xml:lang='ko']">
		          <xsl:for-each select="trans-abstract[@xml:lang='ko']">
<!-- 		            <xsl:call-template name="named-anchor"/> -->
		              <h2 class="article_title">
			            <span><xsl:text>초록</xsl:text></span>
			          </h2>
...
<!-- 		            <xsl:call-template name="named-anchor"/> -->
			...
			<div class="keyword_box">
				<dl>
					<xsl:if test="kwd-group[@xml:lang='ko']">
						<dt>주요 용어</dt>
						<dd>
							<xsl:call-template name="named-anchor"/>
							<xsl:for-each select="kwd-group[@xml:lang='ko']/kwd">
								<span><span class="kwd"><xsl:apply-templates /></span></span>
							</xsl:for-each>
						</dd>
					</xsl:if>
...
<!--       <xsl:if test="article-categories | counts | -->
<!--                     supplementary-material | custom-meta-group | -->
<!--                     custom-meta-wrap"> -->
<!--        <hr class="part-rule"/> -->
<!--         <div class="metadata"> -->
...
<!--   </xsl:template> -->
  ...
  <xsl:template match="history/date" mode="metadata">
    ...
    <xsl:call-template name="format-date"/>
    <xsl:if test="not(position()=last())"><span class="splitter">; </span></xsl:if>
  </xsl:template>
  ...
  <xsl:template match="title-group" mode="metadata">
    ...
    <div class="view_title">
    	<!-- 영문 -->
		<xsl:if test="/article/@xml:lang='en'">
			<h1 lang="ko"><xsl:apply-templates select="trans-title-group[@xml:lang='ko']/trans-title" mode="metadata"/></h1>
		    <h1 lang="en"><xsl:apply-templates select="article-title | subtitle | trans-title-group |	trans-title | trans-subtitle" mode="metadata"/></h1>
		</xsl:if>
		<!-- 국문 -->
		<xsl:if test="/article/@xml:lang='ko'">
			<h1 lang="ko"><xsl:apply-templates select="article-title | subtitle | trans-title-group | trans-title | trans-subtitle" mode="metadata"/></h1>
	        <h1 lang="en"><xsl:apply-templates select="trans-title-group[@xml:lang='en']/trans-title" mode="metadata"/></h1>
	    </xsl:if>
    </div>
      ...
      <xsl:if test="/article/@xml:lang='en'">
	      <xsl:for-each select="contrib">
	        <xsl:if test="position() > 1"><xsl:text>; </xsl:text></xsl:if>
	          <span class="author-aff athor_link">
<!-- 	          <xsl:attribute name="data-rid"><xsl:value-of select="@rid"/></xsl:attribute> -->
	            <xsl:attribute name="data-rid"><xsl:value-of select="position()"/></xsl:attribute>
			    <xsl:call-template name="simple-contrib-identify">
			      <xsl:with-param name="lang">ko</xsl:with-param>
			    </xsl:call-template>
			  </span>
		    <xsl:if test="xref">*</xsl:if>
	      </xsl:for-each>
	      <xsl:if test="contrib/name-alternatives/name[@xml:lang='ko']">
	      	<br/>
	      </xsl:if>
	      <xsl:for-each select="contrib">
		      <xsl:call-template name="simple-bio">
...
<!-- 		      		<xsl:attribute name="data-rid"><xsl:value-of select="@rid"/></xsl:attribute> -->
	      ...
	      <xsl:if test="contrib/name-alternatives/name[@xml:lang='en']">
	      	<br/>
	      </xsl:if>
      </xsl:if>
<!-- 	</li> -->
...
<!-- 	      <p class="misc-contrib-data"> -->
<!-- 	        <xsl:apply-templates mode="metadata" select="$misc-contrib-data"/> -->
<!-- 	      </p> -->
<!--       </xsl:if> -->
	  
<!--       <xsl:apply-templates select="xref" mode="inline" /> -->
  </xsl:template>

  <xsl:template name="simple-bio">
    <xsl:param name="position"/>
    <div class="view_user_toggle hidden">
<!--       <xsl:attribute name="data-rid"><xsl:value-of select="@rid" /></xsl:attribute> -->
      <xsl:attribute name="data-rid"><xsl:value-of select="$position" /></xsl:attribute>
      <p><xsl:value-of select="bio/p"/></p>
      <a href="#" class="ut_close_bt">저자소개 닫기</a>
    </div>
  </xsl:template>
  
  <xsl:template name="simple-contrib-identify">
	<xsl:param name="lang"/>
	<!-- 국문 -->
    <xsl:if test="/article/@xml:lang='ko'">
	  	  <!-- 일반 논문일 경우 -->
		  <xsl:if test="./name-alternatives">
			<xsl:for-each select="name-alternatives/name">
				<xsl:choose>
					<xsl:when test="$lang='' and not(@xml:lang)">
						<xsl:apply-templates mode="metadata-inline"	select="normalize-space()" />
					</xsl:when>
					<xsl:when test="$lang='en' and @xml:lang=$lang">
	...
	<!-- 					<xsl:apply-templates mode="metadata-inline"	select="." /> -->
	        ...
	        <xsl:for-each select="//aff-alternatives[@id=$rid]/aff">
	            <xsl:choose>
					<xsl:when test="$lang='' and not(@xml:lang)">
						<xsl:if test="./institution-wrap">(<xsl:value-of select="./institution-wrap/institution/text()"/>)</xsl:if>
						<xsl:if test="not(./institution-wrap)">(<xsl:value-of select="./text()"/>)</xsl:if>
					</xsl:when>
					<xsl:when test="$lang='en' and @xml:lang=$lang"> (<xsl:value-of select="./text()"/>)</xsl:when>
				</xsl:choose>
	        </xsl:for-each>
	      </xsl:if>
...
<!-- 						<xsl:apply-templates mode="metadata-inline" select="normalize-space()"/> -->
	        ...
	        <xsl:for-each select="//aff[@id=$rid]">
	            <xsl:choose>
					<xsl:when test="$lang='' and not(@xml:lang)">(<xsl:value-of select="./text()" />)</xsl:when>
					<xsl:when test="$lang='en' and @xml:lang=$lang">(<xsl:value-of select="./text()" />)</xsl:when>
				</xsl:choose>
	        </xsl:for-each>
	      </xsl:if>
      ...
      <!-- 영문 -->
  	  <xsl:if test="/article/@xml:lang='en'">
		  ...
		  <xsl:if test="./name-alternatives">
			<xsl:for-each select="name-alternatives/name">
				<xsl:choose>
					<xsl:when test="$lang='ko' and @xml:lang=$lang">
						<xsl:apply-templates mode="metadata-inline"	select="normalize-space()" />
					</xsl:when>
					<xsl:when test="$lang='' and not(@xml:lang)">
						<xsl:value-of select="surname"/>, 
...
<!-- 						<xsl:apply-templates mode="metadata-inline"	select="." /> -->
	...
	<!--         <xsl:call-template name="contrib-identify" /> -->
	        ...
	        <xsl:for-each select="//aff[@id=$rid]">
	            <xsl:choose>
					<xsl:when test="$lang='ko' and @xml:lang=$lang">(<xsl:value-of select="./text()" />)</xsl:when>
					<xsl:when test="$lang='' and not(@xml:lang)">(<xsl:value-of select="./text()" />)</xsl:when>
				</xsl:choose>
	        </xsl:for-each>
	      </xsl:if>
...
<!--   </xsl:template> -->
  ...
  <xsl:template match="author-notes/corresp" mode="metadata">
      <!-- *교신저자(Corresponding Author): <xsl:value-of select="email"/> --> <xsl:apply-templates />
  </xsl:template>
  ...
  <xsl:template match="ref-list" name="ref-list">
    <div class="section ref-list">
      ...
      <xsl:apply-templates select="." mode="label"/>
      <xsl:if test="title">
        <h3 class="section-title no-toc"><xsl:value-of select="title"/></h3>
      </xsl:if>
      <xsl:apply-templates select="*[not(self::ref | self::ref-list | self::title)]"/>
      <xsl:if test="ref">
...
<!--         <xsl:attribute name="style">display: float; clear: both</xsl:attribute> -->
        <xsl:attribute name="class"><xsl:value-of select="$gi" /> text-center</xsl:attribute>
      </xsl:if>
      <xsl:call-template name="named-anchor"/>
      <xsl:apply-templates select="." mode="label"/>
      <xsl:apply-templates/>
...
<!--         <xsl:attribute name="style">display: float; clear: both</xsl:attribute> -->
  ...
  <!-- <xsl:template match="caption_notag">
    <xsl:apply-templates/>
  </xsl:template> -->  
  ...
  <xsl:template match="graphic | inline-graphic">
      ...
      <img id="article-img" alt="{@xlink:href}" style="">
  ...
  <xsl:template match="permissions">
    <div class="permissions">
      ...
      <xsl:if test="not(copyright-statement) and (copyright-year | copyright-holder)">
	      <span class="copyright-year-holder">
  ...
  <xsl:template match="def-list">
    <div class="def-list">
      ...
      <xsl:if test="term-head|def-head">
        <div class="{local-name()}">
          <div class="term-head def-list-head">
            <xsl:apply-templates select="term-head"/>
          </div>
          <div class="def-head def-list-head">
...
<!--     <xsl:apply-templates/> -->
  ...
  <xsl:template match="ref">
      <div class="ref">
        ...
        <xsl:call-template name="named-anchor-div"/>
  ...
  <xsl:template match="aff/label | chem-struct/label |
  ...
  <xsl:template match="mixed-citation/pub-id">
    <xsl:choose>
      ...
      <xsl:when test="@pub-id-type='doi'">
        <a class="doi" target="_tab"><xsl:attribute name="href">https://doi.org/<xsl:value-of select="." /></xsl:attribute>https://doi.org/<xsl:value-of select="." /></a>
      </xsl:when>
      <xsl:when test="@pub-id-type='manuscript'">Manuscript ID</xsl:when>
      <xsl:when test="@pub-id-type='medline'">Medline ID</xsl:when>
      <xsl:when test="@pub-id-type='pii'">Publisher Item ID</xsl:when>
...
<!--       <xsl:call-template name="named-anchor"/> -->
  ...
  <xsl:template match="@style" mode="table-copy" />
  ...
  <xsl:template match="xref">
    <a href="#{@rid}">
      <xsl:attribute name="class"><xsl:text>xref ref-type-</xsl:text><xsl:value-of select="@ref-type" /></xsl:attribute>
      <xsl:apply-templates/>
    </a>
  </xsl:template>
...
<!--     <xsl:if test="fn-group/fn/@fn-type"> -->
<!--       <br/> -->
<!--       <div class="vb_text"> -->
<!--         <dl> -->
<!--           <dt>Acknowledgement</dt> -->
<!--           <dd><xsl:value-of select="fn-group/fn"/></dd> -->
<!--         </dl> -->
<!--       </div> -->
<!--     </xsl:if> -->
...
<!--   </xsl:template> -->
  ...
  <xsl:template match="back/fn-group">
    <xsl:if test="not(fn/@fn-type)">
      <xsl:call-template name="backmatter-section">
        <xsl:with-param name="generated-title">Notes</xsl:with-param>
      </xsl:call-template>
    </xsl:if>
    <xsl:if test="fn/@fn-type">
      <xsl:call-template name="backmatter-section">
        <xsl:with-param name="generated-title">Acknowledgement</xsl:with-param>
      </xsl:call-template>
    </xsl:if>
  </xsl:template>
  ...
  <xsl:template match="fn" mode="footnote">
    <xsl:if test="not(@fn-type='corresp')">
  ...
  <xsl:template match="fn" mode="author-and-history">
    <p class="metadata {@fn-type}">
      <xsl:apply-templates mode="author-and-history"/>
    </p>
  </xsl:template>
  <xsl:template match="fn/p" mode="author-and-history">
    <xsl:if test="not(preceding-sibling::p | preceding-sibling::label)"><br /></xsl:if>
    <xsl:apply-templates/>
  </xsl:template>
...
<!--         <xsl:text>[</xsl:text> -->
        <xsl:choose>
          ...
          <xsl:otherwise><xsl:number level="any" count="fn[not(ancestor::front)]"
            from="article | sub-article | response"/></xsl:otherwise>
        </xsl:choose>
<!--         <xsl:text>]</xsl:text> -->
      </xsl:with-param>
    </xsl:call-template>
...
<!--         <xsl:text>./xref/</xsl:text><xsl:value-of select="."/> -->
        <xsl:value-of select="$base" /><xsl:text>/assets/600x/</xsl:text><xsl:value-of select="."/>
      </xsl:attribute>
    </xsl:for-each>
  </xsl:template>
  
  <xsl:template name="inline-src">
    <xsl:for-each select="@xlink:href">
      <xsl:attribute name="src">
<!--         <xsl:text>./xref/</xsl:text><xsl:value-of select="."/> -->
        <xsl:value-of select="$base" /><xsl:text>/assets/inline/origin/</xsl:text><xsl:value-of select="."/>
      </xsl:attribute>
    </xsl:for-each>
  </xsl:template>
  ...
  <xsl:template name="named-anchor-div">
